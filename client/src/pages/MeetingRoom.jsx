...
const MeetingRoom = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      Object.keys(signalingHandlers).forEach((event) => socketRef.current.off(event))
    }
  }, [isLoaded, cleanup])
  useEffect(() => {
            ...
            <div className="flex justify-center">
              <div className="flex items-center gap-2 sm:gap-4 md:gap-5 p-3 sm:p-4 md:p-5 bg-slate-800/90 backdrop-blur-2xl rounded-xl sm:rounded-2xl md:rounded-3xl border border-slate-700/40 shadow-2xl max-w-fit overflow-x-auto">
                ...
                <button
                  ...
                  className="w-12 h-12 rounded-xl flex items-center justify-center bg-indigo-500/80 text-white border border-indigo-400 border-2 shadow hover:scale-105 transition-all duration-300 flex-shrink-0"
                  title="Translate any word/sentence"
                >
